...
export default function NewOrderPage() {
  ...
  function sortCitiesByPriority(citiesList: any[]) {
    ...
    return [...citiesList].sort((a, b) => {
      const aIndex = priority.indexOf(normalizeCityName(a.name));
      const bIndex = priority.indexOf(normalizeCityName(b.name));
      ...
      <form onSubmit={handleSubmit} className="grid gap-8">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-neutral-400">
                        الكمية: {item.quantity} × {item.sale_price} د.ل
                      </p>
                    </div>
                  </div>
        ...
        {isMayarShippingSelected && (
          <section className="max-w-5xl rounded-2xl border border-blue-700 bg-blue-950/30 p-6">
            ...
            {mayarParcelType === "exchange" && (
              <div className="mt-4 rounded-xl border border-yellow-600 bg-yellow-950/40 p-4 text-sm text-yellow-100">
                تنبيه: في طرد مقابل طرد سيبقى سعر الطلب داخل نظام Elzade كما هو للتقارير والمخزون، لكن سيتم إرسال السعر إلى المعيار بقيمة 0 د.ل حتى لا يدفع الزبون قيمة المنتج مرتين.
              </div>
            )}
          </section>
        )}
